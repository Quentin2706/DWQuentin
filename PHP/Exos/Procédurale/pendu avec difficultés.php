...
function coderMot($t, $difficulte)
{
    if ($difficulte < 2) {
        $t = str_split($t);
...
function testerLettre($lettre, $tab, $depart, $difficulte)
{
    if ($difficulte < 3) {
        ...
        if ($res === false) {
            ...
            $reponse[] = $res + $depart;
            return array_merge($reponse, testerLettre($lettre, $tab, $res + $depart + 1, $difficulte));
        }
    }
}
...
// {
//     echo "Quelle difficulté choisissez vous ?\n";
//     $difficulte = readline("1 étant facile, 2 moyen et 3 difficile :");
//     while (!ctype_digit($difficulte) || $difficulte < 4 xor $difficulte > 0) {
//         echo "\nSaisie invalide.\n";
//         echo "Quelle difficulté choisissez vous ?\n";
//         $difficulte = readline("1 étant facile, 2 moyen et 3 difficile :");
//     }
...
//         if ($difficulte == 1) {
//             while (in_array($lettre, $lettredonnee) || in_array($lettre, $listeMauvaisesLettres) || !ctype_alpha($lettre)) {
...
//             }
//         }
//         $position = testerLettre($lettre, $motTableau, 0, $difficulte);
//         if (empty($position)) {
//             $nberreur++;
